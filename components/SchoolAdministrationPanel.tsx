...
export function SchoolAdministrationPanel({ schoolId, memberships }: { schoolId: string; memberships: SchoolMembership[] }) {
  ...
  const [registeredPeopleOpen, setRegisteredPeopleOpen] = useState(false);
  ...
  return <View style={styles.wrap}>
    ...
    {foundationActive ? <>
      ...
      {tab === "registry" ? <View style={styles.card}>
        ...
        <View style={styles.row}><TextInput style={[styles.input, styles.third]} value={givenName} onChangeText={setGivenName} placeholder="First Name"/><TextInput style={[styles.input, styles.third]} value={middleName} onChangeText={setMiddleName} placeholder="Middle Name (optional)"/><TextInput style={[styles.input, styles.third]} value={familyName} onChangeText={setFamilyName} placeholder="Last Name"/></View>
        <View style={styles.row}>{personType === "student" ? <TextInput style={[styles.input, styles.third]} value={className} onChangeText={setClassName} placeholder="Class"/> : null}{personType !== "guardian" ? <TextInput style={[styles.input, styles.third]} value={identifier} onChangeText={setIdentifier} placeholder={personType === "student" ? "Admission Number" : "Employee Number"}/> : null}<TextInput style={[styles.input, styles.third]} value={email} onChangeText={setEmail} placeholder="Email Address (optional)" autoCapitalize="none"/></View>
        <Pressable disabled={busy} style={styles.primary} onPress={() => void perform(async () => { await createSchoolAdministrationPerson({ schoolId, personType, givenName, familyName, email, identifier, customFields: { middleName: middleName.trim(), className: personType === "student" ? className.trim() : "" } }); setGivenName(""); setMiddleName(""); setFamilyName(""); setClassName(""); setEmail(""); setIdentifier(""); }, "The record was added to the school registry.")}><Text style={styles.primaryText}>Register {personType}</Text></Pressable>
        <Pressable style={styles.collapseHeader} onPress={() => setRegisteredPeopleOpen((current) => !current)} accessibilityRole="button" accessibilityState={{ expanded: registeredPeopleOpen }}><View><Text style={styles.subheadingInline}>Registered people</Text><Text style={styles.meta}>{data?.people.filter((person) => person.status !== "archived").length ?? 0} active records</Text></View><Text style={styles.collapseIcon}>{registeredPeopleOpen ? "▲" : "▼"}</Text></Pressable>
        {registeredPeopleOpen ? <ScrollView horizontal showsHorizontalScrollIndicator><View style={styles.peopleTable}><View style={[styles.peopleRow, styles.peopleHeader]}><Text style={[styles.peopleCell, styles.nameCell, styles.headerText]}>Name</Text><Text style={[styles.peopleCell, styles.classCell, styles.headerText]}>Class</Text><Text style={[styles.peopleCell, styles.dateCell, styles.headerText]}>Date</Text><Text style={[styles.peopleCell, styles.emailCell, styles.headerText]}>Email Address</Text>{data?.viewer.role === "school_owner" ? <Text style={[styles.actionCell, styles.headerText]}>Action</Text> : null}</View>{data?.people.filter((person) => person.status !== "archived").map((person) => { const middle = typeof person.customFields?.middleName === "string" ? person.customFields.middleName.trim() : ""; const studentClass = person.personType === "student" && typeof person.customFields?.className === "string" ? person.customFields.className.trim() : ""; return <View key={person.id} style={styles.peopleRow}><View style={[styles.peopleCell, styles.nameCell]}><Text style={styles.label}>{[person.givenName, middle, person.familyName].filter(Boolean).join(" ")}</Text><Text style={styles.meta}>{person.personType}</Text></View><Text style={[styles.peopleCell, styles.classCell, styles.tableText]}>{studentClass || "—"}</Text><Text style={[styles.peopleCell, styles.dateCell, styles.tableText]}>{new Date(person.createdAt).toLocaleDateString()}</Text><Text style={[styles.peopleCell, styles.emailCell, styles.tableText]}>{person.email || "—"}</Text>{data.viewer.role === "school_owner" ? <View style={styles.actionCell}><Pressable style={styles.danger} onPress={() => Alert.alert("Archive record?", "The record will remain available for audit and recovery.", [{ text: "Cancel" }, { text: "Archive", style: "destructive", onPress: () => void perform(() => archiveSchoolAdministrationPerson({ schoolId, personId: person.id }), "The record was archived.") }])}><Text style={styles.dangerText}>Archive</Text></Pressable></View> : null}</View>; })}</View></ScrollView> : null}</View> : null}
...
const styles = StyleSheet.create({
  ...
  collapseHeader: { marginTop: 20, borderTopWidth: 1, borderTopColor: "#E5EDF1", paddingVertical: 14, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, subheadingInline: { color: palette.navy, fontWeight: "900", fontSize: 17 }, collapseIcon: { color: palette.navy, fontWeight: "900", fontSize: 16 }, peopleTable: { borderWidth: 1, borderColor: "#E5EDF1", borderRadius: 12, overflow: "hidden" }, peopleRow: { flexDirection: "row", alignItems: "center", minWidth: 720, borderTopWidth: 1, borderTopColor: "#E5EDF1", paddingVertical: 10, paddingHorizontal: 8 }, peopleHeader: { backgroundColor: "#F2F7F9", borderTopWidth: 0 }, peopleCell: { paddingHorizontal: 6 }, nameCell: { flex: 1.4, minWidth: 170 }, classCell: { flex: 0.7, minWidth: 90 }, dateCell: { flex: 0.8, minWidth: 105 }, emailCell: { flex: 1.4, minWidth: 190 }, actionCell: { width: 85, paddingHorizontal: 6 }, headerText: { color: palette.navy, fontWeight: "900", fontSize: 12 }, tableText: { color: "#587180", fontSize: 13 },
});

...
Classroom and the portal use the same class ID and persistent classroom store, not copied client-side data. Selecting a class opens its student list. Its results view is a horizontally scrollable student-by-activity table containing only teacher-submitted results. Every result column uses the activity title entered by the teacher; columns are not restricted to predetermined Test, Assignment or Exam labels. The other tabs provide activities and questions, lesson-note content and illustration data, and class messages. The **Results & reports** section handles school-wide collation, reviewed corrections and report sending.

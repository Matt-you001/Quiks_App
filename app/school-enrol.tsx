...
export default function SchoolEnrolScreen() {
  ...
  const fields = useMemo(() => school?.profileFields.filter((field) => field.enabled && field.roles.includes(role)) ?? [], [role, school]);
  ...
  return <AppBackground webContentWidth="narrow">
    <View style={styles.card}><Text style={styles.eyebrow}>QUIKS SCHOOL ENROLMENT</Text><Text style={styles.title}>{school?.name ?? "Find your school"}</Text>
      {!school ? <><TextInput value={code} onChangeText={(value) => setCode(value.toUpperCase())} placeholder="School code" style={styles.input}/><Pressable style={styles.button} onPress={lookup}><Text style={styles.buttonText}>{busy ? "Checking…" : "Find school"}</Text></Pressable></> : <>
